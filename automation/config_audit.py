...
# Devices intentionally OUT of audit scope. R-ISP simulates an external
# ISP's router -- it's not equipment we own or secure, so it's documented
# here as a deliberate exclusion rather than silently dropped.
SKIP = {"R-ISP.txt"}
...
def main(argv):
    ...
    cfg_dir = Path(argv[1])
    all_files = sorted(cfg_dir.glob("*.txt"))
    files = [f for f in all_files if f.name not in SKIP]
    skipped = [f.name for f in all_files if f.name in SKIP]
    if not files:
        print(f"No .txt config files found in {cfg_dir}/")
        return 2

    if skipped:
        print(f"Out of scope (skipped): {', '.join(skipped)}")
